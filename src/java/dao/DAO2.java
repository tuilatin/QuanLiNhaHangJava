...
public class DAO2 {
    ...
    public KhachHang getKhachHangByIdHoaDon(int idHoaDon) {
    String sql = "SELECT kh.ID_KH, kh.TenKH, kh.Ngaythamgia, kh.Doanhso, kh.Diemtichluy, kh.ID_ND, " +
                 "kh.Sdt, dc.ID_DC, dc.SoNhaDuong, dc.PhuongXa, dc.QuanHuyen, dc.TinhThanh, dc.GhiChu, dc.MacDinh " +
                 "FROM HoaDon hd " +
                 "JOIN NguoiDung nd ON hd.ID_ND = nd.ID_ND " +
                 "JOIN KhachHang kh ON nd.ID_ND = kh.ID_ND " +
                 "LEFT JOIN DiaChi dc ON kh.ID_KH = dc.ID_KH AND dc.MacDinh = 1 " +
                 "WHERE hd.ID_HoaDon = ?";
...
//        System.out.println(list);
            DAO2 dao2 =new DAO2();
       KhachHang list2 = dao2.getKhachHangByIdHoaDon(20);
    }
